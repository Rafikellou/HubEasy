...
export default function Header() {
  ...
  const resources = [
    { name: t('resources_client_cases'), href: '/cas-clients', icon: UsersIcon },
    { name: t('resources_salesforce'), href: '/salesforce', icon: CloudIcon },
    { name: t('resources_articles'), href: '/articles', icon: NewspaperIcon },
  ];

  return (
    <header className={cn(
      "fixed top-0 left-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200/50 shadow-sm transition-all duration-500 ease-in-out",
      isOpen ? "md:right-[400px] right-0" : "right-0"
    )}>
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-10">
            <Link href={`/${locale}/integrations`} className="text-gray-600 hover:text-red-500 transition-colors duration-300 font-light">{t('integrations')}</Link>
            <Link href={`/${locale}/tarifs`} className="text-gray-600 hover:text-red-500 transition-colors duration-300 font-light">{t('pricing')}</Link>
            ...
            >
              ...
              >
                <div className="absolute -left-8 top-full z-10 mt-5 w-64 rounded-xl bg-white p-2 shadow-lg ring-1 ring-gray-900/5">
                  <Link
                    href={`/${locale}/agence/notre-vocation`}
                  ...
                  >
                    ...
                    {t('agency_our_mission')}
                  </Link>
                  <Link
                    href={`/${locale}/agence-hubspot`}
                    className="group flex items-center gap-x-4 rounded-lg p-3 text-sm font-light leading-6 text-gray-700 hover:bg-gray-50 hover:text-red-500 transition-all duration-200"
                  >
                    <div className="flex h-10 w-10 flex-none items-center justify-center rounded-lg bg-gray-50 group-hover:bg-red-50 transition-all duration-200">
            ...
            <Link href={`/${locale}/contact`} className="px-4 py-1.5 rounded-full border border-red-500 text-red-600 hover:bg-red-50 transition-colors duration-300 font-medium">{t('contact')}</Link>
        ...
        <div className={`md:hidden transition-all duration-300 ease-in-out ${isMenuOpen
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 bg-white/95 backdrop-blur-md rounded-lg mt-2 border border-gray-200/50 shadow-lg">
            <Link href={`/${locale}/integrations`} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-red-500 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>{t('integrations')}</Link>
            <Link href={`/${locale}/tarifs`} className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-red-500 hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>{t('pricing')}</Link>

            <div className="px-3 py-2 font-medium text-gray-700">{t('agency')}</div>
            <div className="pl-4">
            ...
            <div className="px-3 py-2 font-medium text-gray-700">{t('resources')}</div>
            <div className="pl-4">
              {resources.map((item) => (
                <Link
                  key={item.name}
            ...
            <Link href={`/${locale}/contact`} className="block px-3 py-2 rounded-full border border-red-500 text-red-600 text-base font-medium hover:bg-red-50" onClick={() => setIsMenuOpen(false)}>{t('contact')}</Link>
